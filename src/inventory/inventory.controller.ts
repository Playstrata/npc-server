...
export class InventoryController {
  ...
  async getPlayerInventory(@Param("characterId") characterId: string): Promise<{
    success: boolean;
    data: {
      items: InventoryItem[];
  ...
  async getMaxCarryableQuantity(
    ...
    const maxQuantity =
      await this.inventoryService.calculateMaxCarryableQuantity(
        characterId,
        itemId,
      );
  ...
  async organizeInventory(@Param("characterId") characterId: string): Promise<{
    success: boolean;
    message: string;
  }> {
  ...
  async getInventoryStats(@Param("characterId") characterId: string): Promise<{
    success: boolean;
    data: {
      capacity: CarryingCapacityInfo;
      itemStats: {
    ...
    const itemStats = {
      ...
      equipmentCount: items.filter(
        (item) => item.item.type === "WEAPON" || item.item.type === "ARMOR",
      ).length,
      consumableCount: items.filter((item) => item.item.type === "CONSUMABLE")
        .length,
      materialCount: items.filter((item) => item.item.type === "RAW_MATERIAL")
        .length,
    ...
    const valueStats = {
      totalValue,
      averageItemValue:
        items.length > 0 ? totalValue / itemStats.uniqueItems : 0,
      mostValuableItem,
    };
