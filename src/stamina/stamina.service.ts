...
export class StaminaService {
  ...
  // 耐力消耗配置
  private readonly staminaDrainConfigs: Record<MovementType, StaminaDrainConfig> = {
  ...
  async getStaminaInfo(characterId: string): Promise<StaminaInfo> {
    ...
    if (updatedCharacter!.isResting && updatedCharacter!.restStartTime) {
      restingFor = Math.floor((Date.now() - updatedCharacter!.restStartTime.getTime()) / 1000);
    }

    const timeUntilFull = current < max ? Math.ceil((max - current) / updatedCharacter!.staminaRegenRate) : undefined;
  ...
  async drainStamina(
    ...
    const adjustedDrainRate = config.drainRate + (config.drainRate * config.weightMultiplier * weightPenalty);
    ...
    // 計算新耐力值
    const newStamina = Math.max(0, Math.min(character.maxStamina, character.stamina - totalDrain));
    
    // 計算移動速度懲罰
    const staminaPenalty = this.calculateStaminaMovementPenalty(newStamina, character.maxStamina);
    const weightMovementPenalty = Math.min(0.5, weightPenalty); // 最多50%速度懲罰
    const totalMovementPenalty = Math.min(0.8, staminaPenalty + weightMovementPenalty + config.speedPenalty);
    ...
    this.logger.log(`角色 ${characterId} ${movementType} ${duration}秒，消耗 ${totalDrain.toFixed(1)} 耐力，剩餘 ${newStamina.toFixed(1)}/${character.maxStamina}`);
  ...
  async stopResting(characterId: string): Promise<{
    ...
    if (character.restStartTime) {
      restDuration = Math.floor((Date.now() - character.restStartTime.getTime()) / 1000);
    }
   ...
   * 修改耐力上限（升級時使用）
   */
  async increaseMaxStamina(characterId: string, amount: number): Promise<{
    success: boolean;
    ...
    this.logger.log(`角色 ${characterId} 耐力上限增加 ${amount}，新上限: ${updatedCharacter.maxStamina}`);
  ...
  private async updateCharacterStamina(characterId: string): Promise<void> {
    ...
    if (character.isResting) {
      ...
      const adjustedRegenRate = character.staminaRegenRate * (1 - weightPenalty * 0.5);
      staminaChange = adjustedRegenRate * timeDifference;
    }

    const newStamina = Math.max(0, Math.min(character.maxStamina, character.stamina + staminaChange));

    // 如果耐力已滿且在休息，自動停止休息
    const shouldStopResting = character.isResting && newStamina >= character.maxStamina;
   ...
   * 計算耐力導致的移動速度懲罰
   */
  private calculateStaminaMovementPenalty(currentStamina: number, maxStamina: number): number {
