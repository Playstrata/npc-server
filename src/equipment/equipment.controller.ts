...
export class EquipmentController {
  ...
  async getEquipmentStatus(@Param("characterId") characterId: string): Promise<{
    success: boolean;
    data: EquipmentStatus;
  }> {
    const equipmentStatus =
      await this.equipmentService.getEquipmentStatus(characterId);
  ...
  async calculateCapacity(@Param("characterId") characterId: string): Promise<{
    success: boolean;
  ...
  async getCapacityHelp(): Promise<{
    ...
    return {
      ...
      data: {
        calculation: {
          weightFormula:
            "總容量 = 基礎容量(5kg) + 力量×2 + 體力×0.5 + 背包加成",
          volumeFormula: "總體積 = 基礎體積(2L) + 背包體積加成",
          explanation: {
            baseCapacity:
              "所有角色的基礎攜帶能力都是5公斤，這代表雙手能夠攜帶的重量",
            strengthBonus:
              "每點力量屬性提供2公斤的額外攜帶能力，力量是負重的主要因素",
            vitalityBonus:
              "每點體力屬性提供0.5公斤的額外攜帶能力，體力影響持久力",
            backpackBonus: "裝備背包後根據背包等級提供額外的重量和體積容量",
            volumeCalculation:
              "體積限制確保即使重量足夠，也不能攜帶過多大型物品",
          },
        },
        examples: [
          {
    ...
    const backpacks = [
      {
        ...
        price: 0,
        pros: ["無需購買", "沒有重量負擔", "立即可用"],
        cons: ["容量極小", "格子數量少", "不適合長途旅行", "無法攜帶大型物品"],
      },
      {
