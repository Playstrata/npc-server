...
export class ProfessionService {
  ...
  async performNPCWork(npcConfig: NPCProfessionConfig): Promise<WorkOutput> {
    ...
    this.logger.log(
      `NPC ${npcConfig.npcId} 完成 ${npcConfig.workSchedule.activityType} 工作，產出 ${workOutput.itemsProduced.length} 種物品`,
    );
   ...
   * 執行挖礦工作
   */
  private async performMining(npcConfig: NPCProfessionConfig): Promise<
    Array<{
      itemId: string;
      quantity: number;
      quality: ItemQuality;
  ...
  > {
    ...
    if (npcConfig.productivity.skillLevel >= 40)
      availableOres.push("ore-silver");

    for (const oreId of availableOres) {
      const baseQuantity = npcConfig.productivity.baseRate;
      const actualQuantity = Math.floor(
        baseQuantity * (0.8 + Math.random() * 0.4),
      ); // 80-120% 變動
   ...
   * 執行伐木工作
   */
  private async performWoodcutting(npcConfig: NPCProfessionConfig): Promise<
    Array<{
      itemId: string;
      quantity: number;
      quality: ItemQuality;
  ...
  > {
    ...
    const baseQuantity = npcConfig.productivity.baseRate;
    const actualQuantity = Math.floor(
      baseQuantity * (0.9 + Math.random() * 0.2),
    );
   ...
   * 執行製作工作（鐵匠）
   */
  private async performCrafting(npcConfig: NPCProfessionConfig): Promise<
    Array<{
      itemId: string;
      quantity: number;
      quality: ItemQuality;
  ...
  > {
    ...
    // 檢查是否有足夠的材料製作物品
    const copperOre = availableMaterials.find(
      (item) => item.itemId === "ore-copper",
    );
    const ironOre = availableMaterials.find(
      (item) => item.itemId === "ore-iron",
    );

    // 製作銅劍
    if (copperOre && copperOre.quantity >= 6) {
      // 需要6個銅礦（先做錠再做劍）
      const swordsToMake = Math.floor(copperOre.quantity / 6);
      const actualSwords = Math.min(
        swordsToMake,
        npcConfig.productivity.baseRate,
      );
    ...
    // 製作鐵劍（如果技能等級足夠）
    if (
      npcConfig.productivity.skillLevel >= 30 &&
      ironOre &&
      ironOre.quantity >= 8
    ) {
      ...
      const actualSwords = Math.min(
        swordsToMake,
        Math.floor(npcConfig.productivity.baseRate * 0.5),
      );
  ...
  private async performResearch(
    ...
    const actualExperience = Math.floor(
      baseExperience * (0.9 + Math.random() * 0.2),
    );
      ...
      if (totalQuantity >= npcConfig.inventory.sellThreshold) {
        ...
        for (const customerId of npcConfig.relationships.customers) {
          ...
          if (sellResult.success) {
            this.logger.log(
              `NPC ${npcConfig.npcId} 向 ${customerId} 販售 ${quantityToSell}x ${newItem.itemId}，獲得 ${sellResult.transactionAmount} 金幣`,
            );
            break; // 成功售出後跳出循環
          }
        }
   ...
   * 從供應商採購
   */
  private async attemptPurchaseFromSupplier(
    buyerId: string,
    supplierId: string,
  ): Promise<void> {
    // 檢查供應商的庫存
    ...
    for (const preferredItem of buyerConfig.inventory.preferredItems) {
      const availableItem = supplierInventory.find(
        (item) => item.itemId === preferredItem,
      );
      ...
      if (availableItem && availableItem.quantity >= 5) {
        ...
        if (purchaseResult.success) {
          this.logger.log(
            `NPC ${buyerId} 從 ${supplierId} 採購 ${quantityToBuy}x ${availableItem.itemId}`,
          );
        }
      }
    }
   ...
   * 向客戶販售
   */
  private async attemptSellToCustomer(
    sellerId: string,
    customerId: string,
  ): Promise<void> {
    ...
    for (const item of sellerInventory) {
      if (item.quantity >= 3) {
        ...
        if (sellResult.success) {
          this.logger.log(
            `NPC ${sellerId} 向 ${customerId} 販售 ${quantityToSell}x ${item.itemId}`,
          );
        }
      }
    }
  ...
  private isWorkingTime(npcConfig: NPCProfessionConfig): boolean {
    ...
    const [startHour, startMinute] = npcConfig.workSchedule.startTime
      .split(":")
      .map(Number);
    const [endHour, endMinute] = npcConfig.workSchedule.endTime
      .split(":")
      .map(Number);
   ...
   * 獲取 NPC 庫存（模擬數據）
   */
  private async getNPCInventory(npcId: string): Promise<
    Array<{
      itemId: string;
      quantity: number;
      quality: ItemQuality;
