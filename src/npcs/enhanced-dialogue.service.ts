...
export class EnhancedDialogueService {
   ...
   * 處理玩家的自由文字對話
   */
  async processPlayerDialogue(request: PlayerDialogueRequest): Promise<DialogueResult> {
    this.logger.log(`處理玩家 ${request.playerId} 與 NPC ${request.npcId} 的對話: "${request.playerMessage}"`);

    try {
      // 1. 分析玩家輸入內容
      const contentAnalysis = await this.aiService.analyzePlayerMessage(request.playerMessage);
      
      // 2. 獲取 NPC 詳細資料
      const npcProfile = await this.buildDetailedNPCProfile(request.npcId);
      
      // 3. 獲取關係上下文
      const relationshipContext = await this.getRelationshipContext(request.npcId, request.playerId);
      
      // 4. 建立情境上下文
      const situationalContext = this.buildSituationalContext(request);
      
      // 5. 獲取對話歷史
      const conversationHistory = await this.getRecentConversationHistory(request.npcId, request.playerId, 5);
      ...
      // 8. 檢查 NPC 是否拒絕對話
      const shouldRefuseDialogue = this.shouldRefuseDialogue(contentAnalysis, toleranceCalculation, npcProfile);
      
      if (shouldRefuseDialogue) {
        return this.createRefusalResponse(npcProfile, contentAnalysis, friendshipImpact);
      }
      ...
      // 11. 更新友好度
      const newFriendshipScore = Math.max(0, Math.min(100, 
        relationshipContext.friendshipScore + friendshipImpact.finalImpact
      ));
      
      await this.updateFriendship(request.npcId, request.playerId, newFriendshipScore);
      ...
      this.logger.log(`對話完成，友好度變化: ${friendshipImpact.finalImpact.toFixed(2)}`);
   ...
   * 構建詳細的 NPC 檔案
   */
  private async buildDetailedNPCProfile(npcId: string): Promise<DetailedNPCProfile> {
    // 從現有的 NPC 服務獲取基本資料
    ...
    const detailedProfile: DetailedNPCProfile = {
      ...
      dialoguePreferences: {
        ...
        languagePreference: {
          formality: socialClassMapping.socialClass === SocialClass.NOBILITY ? 'high' : 
                    socialClassMapping.socialClass === SocialClass.SCHOLAR ? 'high' :
                    socialClassMapping.socialClass === SocialClass.ROGUE ? 'low' : 'medium',
          complexity: socialClassMapping.education === EducationLevel.UNIVERSITY ? 'sophisticated' :
                     socialClassMapping.education === EducationLevel.ILLITERATE ? 'simple' : 'moderate'
        }
      },
      ...
      toleranceConfig: {
        inappropriateContent: this.calculateToleranceLevel(socialClassMapping.socialClass, 'inappropriate'),
        nonsensicalText: this.calculateToleranceLevel(socialClassMapping.socialClass, 'nonsensical'),
        lengthTolerance: this.calculateToleranceLevel(socialClassMapping.socialClass, 'length'),
        spamTolerance: this.calculateToleranceLevel(socialClassMapping.socialClass, 'spam'),
        hostilityTolerance: this.calculateToleranceLevel(socialClassMapping.socialClass, 'hostility')
      }
    };
   ...
   * 計算容忍度等級
   */
  private calculateToleranceLevel(socialClass: SocialClass, toleranceType: string): number {
    const baseLevels = {
      [SocialClass.NOBILITY]: { inappropriate: 0, nonsensical: 2, length: 7, spam: 1, hostility: 0 },
      [SocialClass.SCHOLAR]: { inappropriate: 1, nonsensical: 1, length: 9, spam: 2, hostility: 2 },
      [SocialClass.MERCHANT]: { inappropriate: 3, nonsensical: 4, length: 5, spam: 3, hostility: 4 },
      [SocialClass.ARTISAN]: { inappropriate: 4, nonsensical: 5, length: 6, spam: 4, hostility: 5 },
      [SocialClass.COMMONER]: { inappropriate: 5, nonsensical: 6, length: 5, spam: 5, hostility: 6 },
      [SocialClass.ROGUE]: { inappropriate: 8, nonsensical: 7, length: 4, spam: 7, hostility: 8 },
      [SocialClass.OUTLAW]: { inappropriate: 10, nonsensical: 8, length: 3, spam: 8, hostility: 10 }
    };
   ...
   * 獲取近期對話歷史
   */
  private async getRecentConversationHistory(npcId: string, playerId: string, limit: number = 5): Promise<ConversationHistory[]> {
    // 實際應該從資料庫讀取，這裡返回空陣列
    return [];
  }
   ...
   * 檢查是否應該拒絕對話
   */
  private shouldRefuseDialogue(contentAnalysis: ContentAnalysis, toleranceCalculation: any, npcProfile: DetailedNPCProfile): boolean {
    // 如果內容過於不當且超出容忍範圍
    if (contentAnalysis.contentType === 'inappropriate' && toleranceCalculation.finalTolerance < 2) {
      return true;
    }
    
    // 如果是垃圾信息且NPC不容忍
    if (contentAnalysis.contentType === 'spam' && npcProfile.toleranceConfig.spamTolerance < 3) {
      return true;
    }
    
    // 如果內容過長且NPC不耐煩
    if (contentAnalysis.contentFeatures.approximateLength === 'excessive' && 
   ...
   * 創建拒絕回應
   */
  private createRefusalResponse(npcProfile: DetailedNPCProfile, contentAnalysis: ContentAnalysis, friendshipImpact: any): DialogueResult {
  ...
  private async saveConversationHistory(historyData: {
    ...
    try {
      // 實際應該保存到資料庫
      this.logger.log(`保存對話歷史: NPC ${historyData.npcId} <-> Player ${historyData.playerId}`);
    } catch (error) {
