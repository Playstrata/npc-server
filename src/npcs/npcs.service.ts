...
interface NPCData {
  ...
  villageMemory: {
    [playerId: string]: {
      knownEvents: Array<{
        type: 'quest_completed' | 'quest_failed' | 'trade_success' | 'rude_behavior' | 'helpful_action';
        description: string;
...
export class NpcsService {
   ...
   * 獲取NPC的商店
   */
  async getNPCShop(npcId: string, playerId: string): Promise<{
    npcName: string;
    shop: any;
    ...
    const shop = {
      items: visibleItems.map(shopItem => ({
        ...
        price: Math.round(shopItem.adjustedPrice * friendshipInfo.priceModifier),
        ...
        dynamicPriceInfo: shopItem.dynamicPricing ? {
          trend: shopItem.dynamicPricing.demandFactor > 1.1 ? 'rising' : 
                 shopItem.dynamicPricing.demandFactor < 0.9 ? 'falling' : 'stable',
  ...
  async buyFromNPC(
    ...
    if (npc) {
      if (!npc.relationships[playerId]) {
        npc.relationships[playerId] = { reputation: 0, lastInteraction: new Date() };
      }
   ...
   * 獲取玩家與 NPC 的友好度詳細資訊
   */
  async getFriendshipStatus(npcId: string, playerId: string): Promise<{
    npcName: string;
    friendshipInfo: FriendshipInfo;
    relationshipHistory: {
    ...
    if (personalRelationship) {
      ...
      if (personalRelationship.lastInteraction) {
        const daysSinceLastInteraction = Math.floor(
          (new Date().getTime() - personalRelationship.lastInteraction.getTime()) / (1000 * 60 * 60 * 24)
        );
        relationshipHistory.recentActivities.push(`最後互動：${daysSinceLastInteraction}天前`);
      }
    }

    if (villageMemory && villageMemory.knownEvents.length > 0) {
      relationshipHistory.recentActivities.push(`村民們知道你的${villageMemory.knownEvents.length}件事情`);
    }
  ...
  async generateDynamicDialogue(
    ...
    const dialogueContext = {
      ...
      npcWorkStatus: this.getNPCWorkStatus(npcId, context.timeOfDay) as 'working' | 'resting' | 'busy' | 'free',
   ...
   * 映射時間段格式
   */
  private mapTimeOfDay(timeOfDay: string): 'dawn' | 'morning' | 'noon' | 'afternoon' | 'evening' | 'night' {
    const mapping: { [key: string]: 'dawn' | 'morning' | 'noon' | 'afternoon' | 'evening' | 'night' } = {
      'morning': 'morning',
      'afternoon': 'afternoon', 
      'evening': 'evening',
      'night': 'night'
  ...
  async broadcastVillageNews(
    playerId: string, 
    newsType: 'quest_completed' | 'quest_failed' | 'trade_success' | 'rude_behavior' | 'helpful_action',
    sourceNpcId: string,
    description: string,
    impact: number
  ): Promise<void> {
    ...
    console.log(`[VillageNews] 廣播消息: ${newsType} - ${description} (影響: ${impact})`);

    // 遞歸傳播消息
    await this.spreadNewsToNetwork(allNPCs, sourceNpcId, playerId, {
      type: newsType,
      description,
      impact,
      timestamp: new Date(),
      source: sourceNpcId,
      verified: true
    }, spreadRadius, new Set([sourceNpcId]));
  }
  ...
  private async spreadNewsToNetwork(
    ...
    for (const friendId of currentNpc.socialNetwork.friends) {
      ...
      // 根據八卦程度和信任程度調整消息傳播
      const spreadProbability = (friend.socialNetwork.gossipLevel / 10) * 
      ...
      if (Math.random() < spreadProbability) {
        ...
        console.log(`[VillageNews] ${friend.name} 收到來自 ${currentNpc.name} 的消息: ${news.description}`);

        // 繼續傳播
        await this.spreadNewsToNetwork(allNPCs, friendId, playerId, news, remainingRadius - 1, visitedNpcs);
      }
    }
  }
  ...
  private updateOverallImpression(npc: NPCData, playerId: string): void {
    ...
    memory.knownEvents.forEach(event => {
      const daysSince = Math.floor((now.getTime() - event.timestamp.getTime()) / (1000 * 60 * 60 * 24));
    ...
    memory.overallImpression = recentWeight > 0 ? Math.round(totalImpact / recentWeight) : 0;
  ...
  async getVillageReputation(playerId: string): Promise<{
    ...
    allNPCs.forEach(npc => {
      if (npc.villageMemory[playerId]) {
        ...
        // 根據 NPC 影響力加權計算總體聲譽
        const weightedImpression = memory.overallImpression * (npc.socialNetwork.influence / 10);
        totalImpression += weightedImpression;
        npcCount += npc.socialNetwork.influence / 10;
      }
    });

    const overallReputation = npcCount > 0 ? Math.round(totalImpression / npcCount) : 0;
   ...
   * 計算玩家與 NPC 的友好度資訊
   */
  async calculateFriendshipInfo(npcId: string, playerId: string): Promise<FriendshipInfo> {
    const npc = await this.getNPCById(npcId);
    if (!npc) {
      throw new Error('NPC 不存在');
    }
    ...
    // 獲取個人聲譽（直接關係）
    const personalRelationship = npc.relationships[playerId];
    const personalReputation = personalRelationship ? personalRelationship.reputation : 0;

    // 獲取村莊聲譽（間接關係，通過其他 NPC 傳播）
    const villageMemory = npc.villageMemory[playerId];
    const villageReputation = villageMemory ? villageMemory.overallImpression : 0;

    // 計算綜合友好度分數
    // 個人聲譽權重 70%，村莊聲譽權重 30%
    const combinedScore = Math.round(personalReputation * 0.7 + villageReputation * 0.3);
   ...
   * 根據友好度調整商店價格（新的統一方法）
   */
  async getAdjustedPrice(npcId: string, playerId: string, basePrice: number): Promise<{
   ...
   */
  async getAdjustedPriceLegacy(npcId: string, playerId: string, basePrice: number): Promise<number> {
  ...
  async generateReputationBasedDialogue(
    ...
    // 根據聲譽調整稱呼
    let title = this.getReputationBasedTitle(villageReputation.reputationLevel, personalReputation);
    ...
    if (npcMemory && npcMemory.knownEvents.length > 0) {
      const recentEvent = npcMemory.knownEvents[npcMemory.knownEvents.length - 1];
    ...
    // 生成對話選項
    const options = this.generateReputationBasedOptions(villageReputation.reputationLevel, personalReputation);
  ...
  private getReputationBasedTitle(
    ...
    switch (villageLevel) {
      case 'hero': return '英雄';
      case 'popular': return '受歡迎的冒險者';
      case 'neutral': return '冒險者';
      case 'disliked': return '有爭議的旅人';
      case 'enemy': return '不受歡迎的傢伙';
      default: return '陌生人';
    }
  }
