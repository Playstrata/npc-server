...
export class NpcsController {
  ...
  @ApiBody({
    schema: {
      ...
      properties: {
        ...
        newsType: { 
          type: 'string',
          enum: ['quest_completed', 'quest_failed', 'trade_success', 'rude_behavior', 'helpful_action'],
          description: '消息類型'
        },
      ...
      required: ['playerId', 'newsType', 'sourceNpcId', 'description', 'impact']
    }
  })
  @ApiResponse({ status: 200, description: '消息廣播成功' })
  async broadcastNews(@Body() broadcastDto: {
    playerId: string;
  ...
  async getFriendshipStatus(
    ...
    try {
      const result = await this.npcsService.getFriendshipStatus(npcId, playerId);
  ...
  async getNpcTeachableSkills(@Param('npcId') npcId: string) {
    try {
      ...
      // 根據 NPC 職業返回可教授的技能
      const teachableSkills = await this.getTeachableSkillsByNpcProfession(npc.profession, npcId);
  ...
  @ApiOperation({ summary: '獲取 NPC 對特定玩家的可教授技能（含友好度價格調整）' })
  @ApiParam({ name: 'npcId', description: 'NPC ID' })
  @ApiParam({ name: 'playerId', description: '玩家ID' })
  @ApiResponse({
    status: 200,
  ...
  async getNpcTeachableSkillsForPlayer(
    ...
    try {
      ...
      // 獲取友好度資訊
      const friendshipInfo = await this.npcsService.calculateFriendshipInfo(npcId, playerId);

      // 根據 NPC 職業返回可教授的技能
      const baseSkills = await this.getTeachableSkillsByNpcProfession(npc.profession, npcId);
  ...
  @ApiBody({
    schema: {
      ...
      required: ['skillType', 'knowledgeType', 'knowledgeName', 'paymentAmount']
    }
  })
  @ApiResponse({
    status: 200,
  ...
  async teachSkillToPlayer(
    ...
    try {
      ...
      // 驗證 NPC 是否可以教授此技能
      const teachableSkills = await this.getTeachableSkillsByNpcProfession(npc.profession, npcId);
      const canTeach = teachableSkills.find(skill => 
      ...
      // 使用友好度系統計算學習費用
      const friendshipInfo = await this.npcsService.calculateFriendshipInfo(npcId, playerId);
      const adjustedCost = Math.round(canTeach.cost * friendshipInfo.priceModifier);
      ...
      // 生成 NPC 反應對話
      const npcReaction = this.generateTeachingReaction(npc, teachDto.knowledgeName);
      ...
      console.log(`[NpcsController] ${npc.name} 教授 ${teachDto.knowledgeName} 給玩家 ${playerId}`);
  ...
  // 輔助方法
  private async getTeachableSkillsByNpcProfession(profession: string, npcId: string): Promise<any[]> {
