...
export class MaterialsService {
  ...
  async gatherMaterial(
    ...
    // 獲取該地點可採集的材料
    const availableMaterials = this.getAvailableMaterialsAtLocation(
      location,
      gatheringType,
    );
    ...
    for (const material of availableMaterials) {
      const source = material.sources.find(
        (s) => s.method === gatheringType && s.location === location,
      );
      ...
      if (Math.random() * 100 < finalSuccessRate) {
        ...
        // 計算品質
        const quality = this.calculateMaterialQuality(
          luckMultiplier,
          material.rarity,
        );
        ...
        // 添加到玩家背包
        await this.addMaterialToInventory(
          characterId,
          material.id,
          quantity,
          quality,
    ...
    if (totalExperience > 0) {
      await this.updateGatheringSkill(
        characterId,
        gatheringType,
        totalExperience,
      );
    ...
    this.logger.log(
      `[MaterialsService] 角色 ${characterId} 在 ${location} ${this.getGatheringActionName(gatheringType)}，獲得 ${materialsGained.length} 種材料`,
    );
   ...
   * 檢查角色背包中的材料
   */
  async getCharacterMaterials(characterId: string): Promise<
    Array<{
      itemId: string;
      name: string;
      quantity: number;
    ...
    try {
      await this.prisma.$transaction(async (prisma) => {
        for (const [materialId, quantity] of Object.entries(materials)) {
          ...
          if (!inventoryItem) {
            throw new Error(
              `材料不足：${this.getMaterialInfo(materialId)?.name || materialId}`,
            );
          }
      ...
      return {
        success: false,
        message: error instanceof Error ? error.message : "消耗材料失敗",
      };
    }
  }
  ...
  private getAvailableMaterialsAtLocation(
    location: string,
  ...
  private calculateMaterialQuality(
