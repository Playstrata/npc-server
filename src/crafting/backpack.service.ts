...
export class BackpackService {
  ...
  private readonly backpackConfigs: Record<BackpackType, BackpackInfo> = {
    ...
    [BackpackType.WARRIOR_SPECIALIZED]: {
      ...
      requiredMaterials: [
        {
          itemId: "thick-leather",
          name: "厚皮革",
          quantity: 4,
          source: "大型野獸皮革",
        },
        {
          itemId: "metal-reinforcement",
          name: "金屬加固",
          quantity: 3,
          source: "鐵匠製作",
        },
    ...
    [BackpackType.ROGUE_SPECIALIZED]: {
      ...
      requiredMaterials: [
        ...
        {
          itemId: "silent-buckle",
          name: "靜音扣環",
          quantity: 4,
          source: "特殊金屬工藝",
        },
        {
          itemId: "lock-mechanism",
          name: "鎖具機構",
          quantity: 1,
          source: "鐵匠特製",
        },
  ...
  private readonly craftingRecipes: Record<BackpackType, CraftingRecipe> = {
    [BackpackType.SMALL_CLOTH]: {
      ...
      materials:
        this.backpackConfigs[BackpackType.SMALL_CLOTH].requiredMaterials,
      craftingTime: 30,
      successRate: 95,
      experienceGained: 50,
    },
    [BackpackType.MEDIUM_LEATHER]: {
      ...
      materials:
        this.backpackConfigs[BackpackType.MEDIUM_LEATHER].requiredMaterials,
      craftingTime: 60,
      successRate: 85,
    ...
    [BackpackType.LARGE_COMPOSITE]: {
      ...
      materials:
        this.backpackConfigs[BackpackType.LARGE_COMPOSITE].requiredMaterials,
      craftingTime: 120,
      successRate: 75,
    ...
    [BackpackType.MAGE_SPECIALIZED]: {
      ...
      requiredSkillLevel: TailoringSkillLevel.SKILLED,
      materials:
        this.backpackConfigs[BackpackType.MAGE_SPECIALIZED].requiredMaterials,
      craftingTime: 90,
      successRate: 80,
    ...
    [BackpackType.WARRIOR_SPECIALIZED]: {
      ...
      requiredSkillLevel: TailoringSkillLevel.SKILLED,
      materials:
        this.backpackConfigs[BackpackType.WARRIOR_SPECIALIZED]
          .requiredMaterials,
      craftingTime: 100,
      successRate: 78,
    ...
    [BackpackType.ARCHER_SPECIALIZED]: {
      ...
      requiredSkillLevel: TailoringSkillLevel.SKILLED,
      materials:
        this.backpackConfigs[BackpackType.ARCHER_SPECIALIZED].requiredMaterials,
      craftingTime: 85,
      successRate: 82,
    ...
    [BackpackType.ROGUE_SPECIALIZED]: {
      ...
      materials:
        this.backpackConfigs[BackpackType.ROGUE_SPECIALIZED].requiredMaterials,
      craftingTime: 110,
      successRate: 70,
  ...
  async purchaseBackpackFromMerchant(
    ...
    try {
      ...
      this.logger.log(
        `[BackpackService] 角色 ${characterId} 購買背包 ${backpackInfo.name}，花費 ${backpackInfo.price} 金幣`,
      );
  ...
  async getTailoringProgress(characterId: string): Promise<TailoringProgress> {
    ...
    return {
      ...
      reputation: this.calculateReputation(
        tailoringSkill.proficiency,
        Math.floor(tailoringSkill.proficiency / 10) || 0,
      ),
    };
  }
  ...
  async craftBackpack(
    ...
    try {
      const result = await this.prisma.$transaction(async (prisma) => {
        ...
        // 增加裁縫經驗
        await this.addTailoringExperience(
          characterId,
        ...
        if (craftingSuccess) {
          ...
          await this.addTailoringExperience(
            characterId,
            Math.floor(recipe.experienceGained * 0.3),
            prisma,
          );
      ...
      if (result.success) {
        this.logger.log(
          `[BackpackService] 角色 ${characterId} 成功製作 ${backpackType} 背包`,
        );
        return {
          success: true,
  ...
  private getUnlockedRecipes(skillLevel: TailoringSkillLevel): string[] {
    const recipes = [];
    if (skillLevel >= TailoringSkillLevel.NOVICE)
      recipes.push("recipe-small-cloth");
    if (skillLevel >= TailoringSkillLevel.APPRENTICE)
      recipes.push("recipe-medium-leather");
    if (skillLevel >= TailoringSkillLevel.SKILLED) {
      recipes.push(
        "recipe-large-composite",
        "recipe-mage-specialized",
        "recipe-warrior-specialized",
        "recipe-archer-specialized",
      );
    }
    if (skillLevel >= TailoringSkillLevel.EXPERT)
      recipes.push("recipe-rogue-specialized");
    return recipes;
  }
    ...
    if (inventoryItem.quantity === quantity) {
      ...
      await prisma.playerInventory.update({
        ...
        data: {
          ...
          totalVolume:
            (inventoryItem.quantity - quantity) * inventoryItem.volume,
        },
      });
    }
  }
