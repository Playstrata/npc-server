...
export class SkillActivitiesController {
  ...
  async getNearbyGatheringNodes(
    ...
    try {
      ...
      const nearbyNodes =
        await this.gatheringService.getGatheringNodesByLocation(
          mapId,
          xPos,
          yPos,
          searchRadius,
  ...
  async getAllRecipes(@Query("skillType") skillType?: string) {
    try {
      ...
      if (skillType) {
        recipes = await this.craftingService.getRecipesBySkill(
          ...
          relatedActivities: [
            {
              type: "crafting",
              skill: "CRAFTING",
              description: "使用木材製作物品",
            },
            {
              type: "trading",
              skill: "TRADING",
              description: "出售木材賺取金幣",
            },
          ],
        },
        MINING: {
