...
export class SkillsController {
  ...
  async getNpcTeachableSkills(@Param('npcId') npcId: string) {
    try {
      const teachableSkills = await this.skillsService.getNpcTeachableSkills(npcId);
  ...
  async getSkillDetail(
    ...
    try {
      ...
      const nextLevelExp = levelRequirements[skill.level as keyof typeof levelRequirements];
      const expNeeded = nextLevelExp === -1 ? 0 : Math.max(0, nextLevelExp - skill.experience);
      ...
      return {
        ...
        data: {
          ...
          averageProficiency: skill.knowledges.length > 0 
            ? Math.round(skill.knowledges.reduce((sum, k) => sum + k.proficiency, 0) / skill.knowledges.length)
            : 0
        },
        message: '技能詳情獲取成功'
      };
  ...
  async getSkillStatistics(@Param('playerId') playerId: string) {
    try {
      ...
      const totalExperience = Object.values(playerSkills.skills)
        .reduce((sum, skill) => sum + skill.experience, 0);
      const totalKnowledges = Object.values(playerSkills.skills)
        .reduce((sum, skill) => sum + skill.knowledges.length, 0);

      // 技能等級分布
      const skillDistribution = Object.values(playerSkills.skills)
        .reduce((dist, skill) => {
      ...
      const mostPracticedSkill = practiceCount.length > 0 ? practiceCount[0].type : null;

      // 最近活動（最近5次練習）
      const allPracticeHistory = Object.values(playerSkills.skills)
        .flatMap(skill => skill.practiceHistory.map(history => ({
          ...history,
