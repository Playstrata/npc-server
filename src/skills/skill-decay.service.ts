...
export class SkillDecayService {
  ...
  async performDailyDecayCheck(): Promise<void> {
    ...
    try {
      ...
      for (const playerId of allPlayers) {
        const playerDecayEvents = await this.processPlayerSkillDecay(
          playerId,
          decayConfigs,
        );
      ...
      this.logger.log(
        `技能衰減檢查完成。影響玩家: ${totalPlayersAffected}, 總衰減事件: ${totalDecayEvents}`,
      );
    } catch (error) {
   ...
   * 處理單個玩家的技能衰減
   */
  async processPlayerSkillDecay(
    playerId: string,
    decayConfigs: SkillDecayConfig[],
  ): Promise<number> {
    ...
    for (const [skillTypeKey, skill] of Object.entries(playerSkills.skills)) {
      ...
      if (daysSinceLastPractice >= config.decayStartDays) {
        ...
        for (const knowledge of skill.knowledges) {
          ...
          if (newProficiency < originalProficiency) {
            ...
            if (newProficiency <= config.minimumProficiency * 1.5) {
              await this.triggerSkillWarning(
                playerId,
                skillType,
                knowledge.name,
                "CRITICAL",
        ...
        daysSinceLastPractice >=
        config.decayStartDays - config.warningDays
      ) {
        // 提前警告
        for (const knowledge of skill.knowledges) {
          await this.triggerSkillWarning(
    ...
    for (const [skillTypeKey, skill] of Object.entries(playerSkills.skills)) {
      ...
      for (const knowledge of skill.knowledges) {
        if (daysSinceLastPractice >= config.decayStartDays) {
          ...
          const newProficiency = Math.max(
            config.minimumProficiency,
            originalProficiency - decayAmount,
          );
  ...
  async reviewSkill(
    ...
    const restoreAmount = Math.min(
      50,
      maxRestoreProficiency - currentProficiency,
    );
   ...
   * 獲取技能維護建議
   */
  async getSkillMaintenanceRecommendations(playerId: string): Promise<
    Array<{
      skillType: SkillType;
      knowledgeName: string;
  ...
  private async logDecayEvent(
    ...
    try {
      ...
      this.logger.log(
        `[SkillDecay] ${playerId} - ${skillType}:${knowledgeName} - ${eventData.type} 衰減 ${eventData.decayAmount} 點`,
      );
    } catch (error) {
  ...
  private async triggerSkillWarning(
    ...
    // 這裡可以發送通知給玩家
    this.logger.warn(
      `[SkillWarning] ${playerId} - ${skillType}:${knowledgeName} - ${warningLevel}`,
    );
  }
}
