...
export class SkillsService {
  ...
  private calculateSkillLevel(experience: number): SkillLevel {
    ...
    if (experience >= requirements[SkillLevel.GRANDMASTER])
      return SkillLevel.GRANDMASTER;
    ...
    if (experience >= requirements[SkillLevel.JOURNEYMAN])
      return SkillLevel.JOURNEYMAN;
    if (experience >= requirements[SkillLevel.APPRENTICE])
  ...
  async learnSkillFromNpc(
    ...
    // 檢查是否已經學過該知識
    const existingKnowledge = skill.knowledges.find(
      (k) => k.name === knowledgeName,
    );
    ...
    if (
      this.getSkillLevelOrder(skill.level) <
      this.getSkillLevelOrder(requiredLevel)
    ) {
      return {
        success: false,
    ...
    console.log(
      `[SkillsService] 玩家 ${playerId} 從 NPC ${npcId} 學習了 ${knowledgeName}`,
    );
  ...
  async practiceSkill(
    ...
    const baseExperience = this.getBaseExperienceForPractice(
      skillType,
      practiceType,
    );
    const baseProficiency = this.getBaseProficiencyForPractice(
      skillType,
      practiceType,
    );
    ...
    if (knowledgeUsed) {
      ...
      if (knowledge) {
        ...
        proficiencyGained = Math.round(
          proficiencyGained * (1 + knowledgeBonus),
        );

        // 提升該知識的熟練度
        knowledge.proficiency = Math.min(
          1000,
          knowledge.proficiency + Math.round(proficiencyGained * 0.5),
        );
      }
    }
    ...
    console.log(
      `[SkillsService] 玩家 ${playerId} 練習 ${skillType}，獲得 ${experienceGained} 經驗，${proficiencyGained} 熟練度${levelUp ? `，升級到 ${skill.level}` : ""}`,
    );
   ...
   * 獲取 NPC 可教授的技能知識
   */
  async getNpcTeachableSkills(npcId: string): Promise<
    Array<{
      skillType: SkillType;
      knowledgeType: KnowledgeType;
      knowledgeName: string;
  ...
  private getBaseExperienceForPractice(
    skillType: SkillType,
    practiceType: string,
  ): number {
    // 不同技能的基礎經驗獲得
  ...
  private getBaseProficiencyForPractice(
    skillType: SkillType,
    practiceType: string,
  ): number {
    // 不同技能的基礎熟練度獲得
