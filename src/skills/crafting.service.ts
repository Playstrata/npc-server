...
export class CraftingService {
  ...
  async getAllRecipes(): Promise<CraftingRecipe[]> {
    return [
      // 鍛造配方
      {
        ...
        baseSuccessRate: 0.8,
        outputs: [{ itemType: "IRON_SWORD", quantity: 1, quality: "normal" }],
        experienceReward: 50,
      },
      {
        ...
        baseSuccessRate: 0.6,
        outputs: [{ itemType: "STEEL_ARMOR", quantity: 1, quality: "good" }],
      ...
      // 煉金配方
      {
        ...
        tools: [{ toolType: "ALCHEMY_KIT", durabilityConsumed: 5 }],
      ...
      },
      {
        ...
        tools: [{ toolType: "ALCHEMY_KIT", durabilityConsumed: 8 }],
        craftingTime: 240, // 4分鐘
        baseSuccessRate: 0.75,
        outputs: [{ itemType: "MANA_POTION", quantity: 1, quality: "good" }],
        experienceReward: 60,
      },
      ...
      // 烹飪配方
      {
        ...
        tools: [{ toolType: "COOKING_FIRE", durabilityConsumed: 0 }],
        craftingTime: 60, // 1分鐘
        baseSuccessRate: 0.95,
        outputs: [{ itemType: "GRILLED_FISH", quantity: 1, quality: "normal" }],
        experienceReward: 15,
      },
    ];
  }
  ...
  async performCrafting(
    ...
    if (recipe.requiredKnowledge) {
      const hasKnowledge = skill.knowledges.some(
        (k) => k.name === recipe.requiredKnowledge,
      );
      if (!hasKnowledge) {
        return {
          success: false,
    ...
    // 成功製造
    const craftingResult = this.calculateCraftingResult(
      recipe,
      skill,
      quantity,
      useHighQualityMaterials,
    ...
    console.log(
      `[CraftingService] 玩家 ${playerId} 成功製造了 ${quantity} 個 ${recipe.name}`,
    );
  ...
  private calculateSuccessRate(
    ...
    if (recipe.requiredKnowledge) {
      const knowledge = skill.knowledges.find(
        (k: any) => k.name === recipe.requiredKnowledge,
      );
      if (knowledge) {
  ...
  private calculateCraftingResult(
    ...
    for (let i = 0; i < quantity; i++) {
      ...
      itemsCreated.push({
        ...
        quality,
        durability: this.calculateInitialDurability(
          recipe.outputs[0].itemType,
          quality,
        ),
      });
    }
  ...
  private calculateInitialDurability(
    itemType: string,
    quality: string,
  ): number {
