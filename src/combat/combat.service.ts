...
import { MONSTERS_DATABASE, getMonstersForLevel, getMonstersForLocation } from './monsters.database';
...
export class CombatService {
   ...
   * 開始戰鬥
   */
  async startCombat(playerId: string, startCombatDto: StartCombatDto): Promise<CombatInstance> {
    // 檢查玩家是否已在戰鬥中
    ...
    // 檢查位置限制
    if (startCombatDto.playerLocation && 
        !monsterDef.spawnLocations.includes(startCombatDto.playerLocation)) {
      throw new BadRequestException('該怪物不會在此位置出現');
    }
    
    // 創建怪物實例
    const monsterInstance = this.createMonsterInstance(monsterDef, startCombatDto.playerLocation);
    
    // 計算玩家戰鬥屬性
    const playerCombatStats = this.combatEngine.calculatePlayerCombatStats(character);
  ...
  async executeCombatAction(
    ...
    // 執行戰鬥回合
    const { events, combatContinues } = await this.combatEngine.executeCombatRound(
   ...
   * 獲取適合玩家等級的怪物列表
   */
  async getAvailableMonsters(playerId: string, location?: string): Promise<MonsterDefinition[]> {
    const character = await this.prisma.gameCharacter.findUnique({
      where: { userId: playerId }
    });
   ...
   * 生成戰鬥結果
   */
  private async generateCombatResult(combat: CombatInstance): Promise<CombatResult> {
    const playerVictory = combat.monsterInstance.state === MonsterState.DEAD;
    const combatDuration = (combat.endTime!.getTime() - combat.startTime.getTime()) / 1000;
    ...
    if (playerVictory) {
      ...
      const levelDiff = combat.monsterDefinition.stats.level - combat.playerCombatStats.level;
      const experienceMultiplier = Math.max(0.5, Math.min(1.5, 1 + (levelDiff * 0.1)));
      
      const baseExp = combat.monsterDefinition.rewards.baseExperience;
      const finalExp = Math.round(baseExp * experienceMultiplier);
      
      // 計算掉落物品
      const droppedItems = this.calculateDrops(combat.monsterDefinition.rewards.drops);
      ...
      rewards = {
        experience: finalExp,
        combatSkillExperience: combat.monsterDefinition.rewards.combatSkillExperience,
        gold: goldReward,
        items: droppedItems,
   ...
   * 應用戰鬥結果到玩家
   */
  private async applyCombatResult(playerId: string, result: CombatResult): Promise<void> {
    if (result.playerVictory && result.rewards) {
      ...
      if (result.rewards.specialRewards) {
        if (result.rewards.specialRewards.luckBonus) {
          await this.luckService.updateLuck(playerId, LuckEvent.RARE_DISCOVERY, {
   ...
   * 增加角色經驗值和處理升級
   */
  private async addCharacterExperience(playerId: string, experience: number): Promise<void> {
    const character = await this.prisma.gameCharacter.findUnique({
      where: { userId: playerId }
    });
    ...
    if (newExperience >= requiredExp) {
      ...
      // 升級時回復生命值和魔力
      const characterClassData = getClassData(character.characterClass as CharacterClass);
      
      const newMaxHealth = character.maxHealth + characterClassData.growthRates.healthPerLevel;
      const newMaxMana = character.maxMana + characterClassData.growthRates.manaPerLevel;
   ...
   * 增加角色金幣
   */
  private async addCharacterGold(playerId: string, gold: number): Promise<void> {
    await this.prisma.gameCharacter.update({
      where: { userId: playerId },
      data: {
        goldAmount: {
