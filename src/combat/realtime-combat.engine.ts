...
export class RealtimeCombatEngine {
   ...
   * 更新戰鬥房間狀態 (主要更新循環)
   */
  updateCombatRoom(room: RealtimeCombatRoom, deltaTime: number): RealtimeCombatEvent[] {
    const events: RealtimeCombatEvent[] = [];
    const currentTime = new Date();
    
    // 更新所有玩家狀態
    for (const [playerId, player] of room.players) {
      const playerEvents = this.updatePlayerState(player, room, deltaTime, currentTime);
      events.push(...playerEvents);
    }
    
    // 更新所有怪物狀態
    for (const [monsterId, monster] of room.monsters) {
      const monsterEvents = this.updateMonsterState(monster, room, deltaTime, currentTime);
  ...
  processPlayerAction(
    ...
    switch (action.type) {
      case 'MOVE':
        const moveEvents = this.handlePlayerMovement(player, action, room, currentTime);
        events.push(...moveEvents);
        break;
        
      case 'ATTACK':
        const attackEvents = this.handlePlayerAttack(player, action, room, currentTime);
        events.push(...attackEvents);
        break;
        
      case 'CAST_SKILL':
        const skillEvents = this.handlePlayerSkill(player, action, room, currentTime);
  ...
  private handlePlayerAttack(
    ...
    // 檢查是否在攻擊冷卻中
    if (player.attack.cooldownEndTime && currentTime < player.attack.cooldownEndTime) {
      return events;
    }
    
    // 檢查耐力是否足夠
    const staminaCost = this.calculateStaminaCost(action.data.attackType || 'MELEE');
    ...
    // 設置攻擊狀態
    const attackDuration = this.getAttackDuration(action.data.attackType || 'MELEE');
  ...
  private updatePlayerState(
    ...
    if (player.movement.isMoving) {
      const moveEvents = this.updatePlayerMovement(player, room, deltaTime, currentTime);
      events.push(...moveEvents);
    }
    
    // 更新攻擊
    if (player.attack.isAttacking) {
      const attackEvents = this.updatePlayerAttack(player, room, deltaTime, currentTime);
      events.push(...attackEvents);
    }
    
    // 恢復耐力
    const staminaRegenEvents = this.updatePlayerStamina(player, deltaTime, currentTime);
  ...
  private updatePlayerMovement(
    ...
    // 計算新位置
    const newPosition = {
      x: player.movement.position.x + (player.movement.velocity.x * deltaTime / 1000),
      y: player.movement.position.y + (player.movement.velocity.y * deltaTime / 1000)
    };
    ...
    // 碰撞檢查 (與其他實體)
    const collision = this.checkCollisionWithEntities(newPosition, player, room);
  ...
  private updatePlayerAttack(
    ...
    // 檢查攻擊動作是否完成
    if (player.attack.attackStartTime && 
        currentTime.getTime() - player.attack.attackStartTime.getTime() >= player.attack.attackDuration) {
  ...
  private executeAttackHitCheck(
    ...
    for (const [monsterId, monster] of room.monsters) {
      ...
      if (collision.hasCollision) {
        ...
        if (hitResult.isHit) {
          // 造成傷害
          monster.currentStats.health = Math.max(0, 
  ...
  private updateMonsterState(
    ...
    // 更新AI行為
    const aiEvents = this.updateMonsterAI(monster, room, deltaTime, currentTime);
    events.push(...aiEvents);
    
    // 更新移動
    if (monster.movement.isMoving) {
      const moveEvents = this.updateMonsterMovement(monster, room, deltaTime, currentTime);
      events.push(...moveEvents);
    }
    
    // 更新攻擊
    if (monster.attack.isAttacking) {
      const attackEvents = this.updateMonsterAttack(monster, room, deltaTime, currentTime);
      events.push(...attackEvents);
    }
  ...
  private updateMonsterAI(
    ...
    switch (monster.ai.currentBehavior) {
      case 'IDLE':
        if (nearestPlayer && this.getDistance(monster.movement.position, nearestPlayer.movement.position) <= monster.combatStats.aggroRange) {
          // 切換到追擊模式
      ...
      case 'PATROL':
        const patrolEvents = this.handleMonsterPatrol(monster, room, currentTime);
      ...
      case 'ATTACK':
        const attackEvents = this.handleMonsterAttackBehavior(monster, room, currentTime);
        events.push(...attackEvents);
        break;
      ...
      case 'RETURN':
        const returnEvents = this.handleMonsterReturn(monster, room, currentTime);
   ...
   * 邊界框碰撞檢測
   */
  private checkBoundingBoxCollision(box1: BoundingBox, box2: BoundingBox): CollisionResult {
    const hasCollision = box1.x < box2.x + box2.width &&
    ...
    return {
      hasCollision,
      distance: hasCollision ? 0 : this.getDistance(
  ...
  // TODO: 實作其餘輔助方法
  private handlePlayerSkill(player: RealtimePlayerState, action: PlayerInputAction, room: RealtimeCombatRoom, currentTime: Date): RealtimeCombatEvent[] { return []; }
  private handlePlayerStop(player: RealtimePlayerState, room: RealtimeCombatRoom, currentTime: Date): RealtimeCombatEvent[] { return []; }
  private updatePlayerStamina(player: RealtimePlayerState, deltaTime: number, currentTime: Date): RealtimeCombatEvent[] { return []; }
  private calculateStaminaCost(attackType: string): number { return 10; }
  private getAttackDuration(attackType: string): number { return 500; }
  private checkCollisionWithEntities(position: Vector2D, player: RealtimePlayerState, room: RealtimeCombatRoom): CollisionResult { return { hasCollision: false, distance: 0 }; }
  private calculateAttackHit(player: RealtimePlayerState, monster: RealtimeMonsterState, currentTime: Date): AttackHitResult { 
    return { 
      isHit: true, 
      isDodged: false, 
      damage: 50, 
  ...
  private addMonsterAggro(monster: RealtimeMonsterState, playerId: string, damage: number): void {}
  private updateMonsterMovement(monster: RealtimeMonsterState, room: RealtimeCombatRoom, deltaTime: number, currentTime: Date): RealtimeCombatEvent[] { return []; }
  private updateMonsterAttack(monster: RealtimeMonsterState, room: RealtimeCombatRoom, deltaTime: number, currentTime: Date): RealtimeCombatEvent[] { return []; }
  private findNearestPlayer(monster: RealtimeMonsterState, room: RealtimeCombatRoom): RealtimePlayerState | null { return null; }
  private handleMonsterPatrol(monster: RealtimeMonsterState, room: RealtimeCombatRoom, currentTime: Date): RealtimeCombatEvent[] { return []; }
  private handleMonsterChase(monster: RealtimeMonsterState, room: RealtimeCombatRoom, currentTime: Date): RealtimeCombatEvent[] { return []; }
  private handleMonsterAttackBehavior(monster: RealtimeMonsterState, room: RealtimeCombatRoom, currentTime: Date): RealtimeCombatEvent[] { return []; }
  private handleMonsterFlee(monster: RealtimeMonsterState, room: RealtimeCombatRoom, currentTime: Date): RealtimeCombatEvent[] { return []; }
  private handleMonsterReturn(monster: RealtimeMonsterState, room: RealtimeCombatRoom, currentTime: Date): RealtimeCombatEvent[] { return []; }
  private updateStatusEffects(room: RealtimeCombatRoom, deltaTime: number, currentTime: Date): RealtimeCombatEvent[] { return []; }
  private cleanupDeadEntities(room: RealtimeCombatRoom, currentTime: Date): RealtimeCombatEvent[] { return []; }
}