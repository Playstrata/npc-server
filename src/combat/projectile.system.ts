...
import { Vector2D, BoundingBox, RealtimeCombatEvent, RealtimeCombatEventType } from './realtime-combat.types';
...
export class ProjectileSystem {
  ...
  private projectileDefinitions: Map<ProjectileType, ProjectileDefinition> = new Map();
  ...
  createArcherProjectile(
    ...
    targetId?: string,
    targetType?: 'PLAYER' | 'MONSTER'
  ): { projectile?: ProjectileState; rangeResult: RangeCalculationResult; error?: string } {
    // 計算射程
  ...
  calculateArcherRange(params: ArcherAttackParams): RangeCalculationResult {
    const distance = this.getDistance(params.attackerPosition, params.targetPosition);
    ...
    if (distance > maxRange) {
      ...
      rangeCategory = 'LONG';
      const falloffRatio = (distance - effectiveRange) / (maxRange * 0.9 - effectiveRange);
      ...
      accuracyModifier = (1.0 - weaponConfig.accuracyFalloff) * (1.0 - falloffRatio * 0.5);
      damageModifier = (1.0 - weaponConfig.damageFalloff) * (1.0 - falloffRatio * 0.3);
    }
  ...
  private getWeaponRangeConfig(weaponType: string): WeaponRangeConfig {
    return this.weaponRangeConfigs.get(weaponType) || {
      baseRange: 200,
   ...
   * 確定箭矢類型
   */
  private determineArrowType(params: ArcherAttackParams, rangeResult: RangeCalculationResult): ProjectileType {
    // 根據距離和武器類型決定箭矢類型
    if (rangeResult.rangeCategory === 'EXTREME' || rangeResult.rangeCategory === 'LONG') {
  ...
  private applyArcherModifications(
    ...
    // 根據射程調整飛行速度
    if (rangeResult.rangeCategory === 'LONG' || rangeResult.rangeCategory === 'EXTREME') {
  ...
  updateProjectiles(
    ...
    for (const [projectileId, projectile] of this.projectiles) {
      ...
      // 檢查生存時間
      if (currentTime.getTime() - projectile.creationTime.getTime() > projectile.lifespan) {
  ...
  private updateProjectileBehavior(
    ...
    switch (projectile.behavior) {
      ...
      case ProjectileBehavior.HOMING:
        const homingEvents = this.updateHomingBehavior(projectile, players, monsters);
  ...
  private updateHomingBehavior(
    ...
    // 應用追蹤力
    const homingStrength = projectile.homingStrength * (1 / (distance * 0.01 + 1)); // 距離越近追蹤越強
    ...
    // 更新方向
    projectile.direction = Math.atan2(projectile.velocity.y, projectile.velocity.x) * (180 / Math.PI);
   ...
   * 更新拋物線行為
   */
  private updateParabolicBehavior(projectile: ProjectileState, deltaTime: number): void {
    // 應用重力
  ...
  private updateProjectilePosition(
    ...
    if (!this.isPositionInBounds(projectile.position, mapBounds)) {
      if (projectile.behavior === ProjectileBehavior.BOUNCING && 
  ...
  private checkProjectileCollisions(
    ...
    for (const [targetId, target] of targets) {
      ...
      const collision = this.checkBoundingBoxCollision(projectile.hitbox, targetBoundingBox);
  ...
  private handleProjectileHit(
    ...
    if (projectile.damageType === 'HEAL') {
      ...
      events.push({
        ...
        [projectile.sourceType === 'PLAYER' ? 'playerId' : 'monsterInstanceId']: projectile.sourceId,
        [projectile.targetType === 'PLAYER' ? 'playerId' : 'monsterInstanceId']: targetId,
      ...
      // 傷害效果
      target.currentStats.health = Math.max(0, target.currentStats.health - finalDamage);
      ...
      if (target.currentStats.health <= 0) {
        const deathEventType = projectile.targetType === 'PLAYER' ? 
    ...
    if (projectile.statusEffects.length > 0) {
      ...
      for (const effect of projectile.statusEffects) {
        ...
        const fullStatusEffect = {
          id: this.generateEventId(),
          type: effect.type.includes('HEAL') ? 'HOT' : 'DEBUFF' as 'BUFF' | 'DEBUFF' | 'DOT' | 'HOT',
          name: effect.type,
  ...
  private handleExplosion(
    ...
    for (const [targetId, target] of allTargets) {
      ...
      if (distance <= projectile.explosionRadius) {
        // 計算距離衰減傷害
        const damageMultiplier = Math.max(0.3, 1 - (distance / projectile.explosionRadius));
        const explosionDamage = Math.round(projectile.damage * damageMultiplier);
        
        // 造成傷害
        target.currentStats.health = Math.max(0, target.currentStats.health - explosionDamage);
  ...
  private calculateHitbox(position: Vector2D, type: ProjectileType): BoundingBox {
    let size = 8; // 默認大小
  ...
  private checkBoundingBoxCollision(box1: BoundingBox, box2: BoundingBox): { hasCollision: boolean } {
    return {
      hasCollision: box1.x < box2.x + box2.width &&
  ...
  private handleBounce(projectile: ProjectileState, mapBounds: BoundingBox): void {
    // 牆壁彈跳邏輯
    if (projectile.position.x <= mapBounds.x || 
        projectile.position.x >= mapBounds.x + mapBounds.width) {
      projectile.velocity.x = -projectile.velocity.x;
    }
    
    if (projectile.position.y <= mapBounds.y || 
  ...
  private calculateProjectileDamage(projectile: ProjectileState, target: any): number {
  ...
  isTargetInRange(
    archerParams: ArcherAttackParams
  ): { inRange: boolean; distance: number; maxRange: number; rangeCategory: string } {
    const rangeResult = this.calculateArcherRange(archerParams);
