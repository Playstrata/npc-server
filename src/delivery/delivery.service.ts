...
export class DeliveryService {
  ...
  async generateDeliveryQuests(): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`生成了 ${generatedQuests} 個新的送貨任務，當前可用任務總數: ${existingQuestCount + generatedQuests}`);
    } catch (error) {
  ...
  private async generateRandomDeliveryQuest(): Promise<DeliveryQuestConfig | null> {
    ...
    // 隨機選擇物品
    const availableItems = ['ore-copper', 'ore-iron', 'wood-oak', 'weapon-copper-sword'];
    ...
    try {
      ...
      this.logger.log(`生成送貨任務: ${questConfig.fromLocation} → ${questConfig.toLocation}, ${quantity}x ${itemId}`);
      return questConfig;
    } catch (error) {
   ...
   * 執行 NPC 送貨員工作
   */
  private async performWorkerDelivery(worker: NPCDeliveryWorkerConfig): Promise<number> {
    // 尋找適合此送貨員的任務
    ...
    // 選擇最適合的任務 (考慮路線偏好和效率)
    const selectedQuest = this.selectBestQuestForWorker(availableQuests, worker);
    if (!selectedQuest) {
      return 0;
    }
    ...
    }, Math.min(completionTime * 60 * 1000, 60000)); // 最多等待1分鐘 (演示用)

    this.logger.log(`NPC送貨員 ${worker.name} 開始執行任務: ${selectedQuest.fromLocation} → ${selectedQuest.toLocation}`);
    return 1;
  }
   ...
   * 完成 NPC 送貨
   */
  private async completeNPCDelivery(questId: string, workerId: string): Promise<void> {
    try {
      await this.prisma.deliveryQuest.update({
        where: { id: questId },
        data: {
  ...
  async getAvailableDeliveryQuests(
    ...
    const availableCapacity = character.carryingCapacity - character.currentWeight;
    const weightLimit = maxWeight ? Math.min(maxWeight, availableCapacity) : availableCapacity;
  ...
  async acceptDeliveryQuest(
    ...
    // 檢查玩家負重能力
    const capacityInfo = await this.inventoryService.getCarryingCapacityInfo(characterId);
   ...
   * 計算任務難度
   */
  private calculateDifficulty(weight: number, distance: number): DeliveryDifficulty {
   ...
   * 計算時間限制
   */
  private calculateTimeLimit(distance: number, difficulty: DeliveryDifficulty): number {
   ...
   * 計算獎勵
   */
  private calculateRewards(weight: number, distance: number, difficulty: DeliveryDifficulty) {
  ...
  private selectBestQuestForWorker(
    ...
    for (const quest of quests) {
      ...
      // 偏好路線加分
      if (worker.preferredRoutes.includes(quest.fromLocation) || 
   ...
   * 計算 NPC 送貨完成時間
   */
  private calculateCompletionTime(quest: any, worker: NPCDeliveryWorkerConfig): number {
  ...
  private isWorkingTime(worker: NPCDeliveryWorkerConfig): boolean {
    ...
    const [startHour, startMinute] = worker.workSchedule.startTime.split(':').map(Number);
    const [endHour, endMinute] = worker.workSchedule.endTime.split(':').map(Number);
