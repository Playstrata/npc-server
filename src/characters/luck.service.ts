...
export class LuckService {
  ...
  async triggerLuckEvent(
    ...
    this.logger.log(
      `[LuckService] 角色 ${character.characterName} 觸發幸運事件: ${message}`,
    );
  ...
  async applyLuckToAction(
    ...
    // 應用幸運值到成功率
    const modifiedSuccessRate = Math.min(
      100,
      Math.max(0, baseSuccessRate * luckMultiplier),
    );
   ...
   * 計算幸運對爆擊率的影響
   */
  async calculateLuckyCriticalRate(
    characterId: string,
    baseCritRate: number,
  ): Promise<{
  ...
  async processDailyLuckDecay(): Promise<void> {
    ...
    this.logger.log(
      `[LuckService] 處理了 ${characters.length} 個角色的每日幸運值衰減`,
    );
  }
  ...
  async updateLuck(characterId: string, event: LuckEvent, context?: any) {
    try {
      ...
      const newLuckPercentage = Math.max(
        0,
        Math.min(100, character.luckPercentage + luckChange),
      );
      ...
      this.logger.log(
        `角色 ${characterId} 幸運值變化: ${event} ${luckChange > 0 ? "+" : ""}${luckChange}%`,
      );
  ...
  private getLuckChangeForEvent(event: LuckEvent): number {
    switch (event) {
      case LuckEvent.SKILL_CRITICAL_SUCCESS:
        return 0.5;
      case LuckEvent.COMBAT_VICTORY:
        return 0.2;
      case LuckEvent.QUEST_COMPLETION:
        return 0.3;
      case LuckEvent.RARE_ITEM_FOUND:
        return 1.0;
      case LuckEvent.RARE_DISCOVERY:
        return 1.0;
      case LuckEvent.SUCCESSFUL_TRADE:
        return 0.1;
      case LuckEvent.HELPING_OTHERS:
        return 0.4;
      case LuckEvent.LEVEL_UP:
        return 0.3;
      case LuckEvent.JOB_CHANGE_SUCCESS:
        return 2.0;
      case LuckEvent.SKILL_CRITICAL_FAILURE:
        return -0.8;
      case LuckEvent.COMBAT_DEFEAT:
        return -0.5;
      case LuckEvent.QUEST_FAILURE:
        return -0.7;
      case LuckEvent.ITEM_BROKEN:
        return -0.3;
      case LuckEvent.BAD_TRADE:
        return -0.2;
      case LuckEvent.ANTISOCIAL_BEHAVIOR:
        return -1.0;
      case LuckEvent.DEATH:
        return -3.0;
      case LuckEvent.STEALING:
        return -1.5;
      case LuckEvent.LYING_TO_NPC:
        return -0.6;
      case LuckEvent.DAILY_DECAY:
        return -0.1;
      default:
        return 0;
    }
